...
import LiquidGaugeTank from "./LiquidGaugeTank";
import { useAppDispatch, useAppSelector } from "../../../store/store";
import { getDeviceById } from "../../../store/deviceSlice";
import {
  getDeviceTankLogs,
  getTankCustomReport,
} from "../../../store/tankDeviceSlice";
...
const TankDevice = () => {
  ...
  const totalItems = useMemo(() => {
    ...
    return 0;
  }, [runTimeData, customReportData, selectedReport]);

  const selectedRows = useMemo(() => {
    let currentData: TankDeviceResultItem[] = [];
    if (selectedReport === "runTime") {
  ...
  const tankPercentage = useMemo(() => {
    return (
      ((deviceData?.last_record?.last_level || 0) * 100) /
      deviceData?.params?.height
    );
  }, [deviceData]);
  ...
  const downloadRunTimeCSV = () => {
    ...
    const useArray = runTimeData.map((data, index) => ({
      ...
      "Available Water(Start) (Ltr)": data.first_level,
      "Available Water(End) (Ltr)": data.last_level,
    }));

    downloadCSV(useArray, `Tank_Run_Time_Report_${runTimeDate || "data"}`);
  };
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-fit">
                <div className="bg-primary border border-border-primary rounded-lg px-4 py-2 h-fit">
                  <div className="flex items-center gap-2">
                    <Waves className="w-5 h-5 text-status-info" />
                    <span className="text-xl font-normal text-text-primary font-roboto">
                      Water Storage Tank
                    </span>
                  </div>
                  ...
                  <LiquidGaugeTank
                    ...
                    capacity={deviceData?.params?.storageCapacity || 0}
                  />
                </div>

                <div className="bg-primary border border-border-primary rounded-lg overflow-hidden h-fit">
                  <h2 className="text-text-primary p-2 flex items-center gap-2 font-roboto text-xl font-normal">
                    <span className="text-status-danger text-xs font-roboto">
            ...
            {selectedReport === "none" ? (
              <div className="rounded-lg h-full flex flex-col gap-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  >
                    <span className="text-text-primary font-normal text-lg font-roboto">
                      Run Time Report
                    </span>
              ...
              <div className="rounded-lg h-full flex flex-col gap-3">
                <div className="flex items-start md:items-center flex-col md:flex-row justify-between md:gap-4 gap-2">
                  <h2 className="text-xl font-normal text-text-primary font-roboto flex items-center gap-2">
                    <ChevronsLeft
                      onClick={handleBack}
                            ...
                            handlePaginatedData(runTimeData)?.map(
                              (data, index) => (
                                ...
                                >
                                  ...
                                  <td className="px-4 py-3 text-text-primary text-start font-roboto text-base capitalize">
                                    {deviceData?.unit === "M^3"
                                      ? (data?.first_level / 1000).toFixed(2)
                                      : data?.first_level || 0}
                                  </td>
                                  <td className="px-4 py-3 text-text-primary text-start font-roboto text-base capitalize">
                                    {deviceData?.unit === "M^3"
                                      ? (data?.last_level / 1000).toFixed(2)
                                      : data?.last_level || 0}
                                  </td>
                                </tr>
                              )
                  ...
                  <div className="flex items-start md:items-end flex-col md:flex-row flex-wrap gap-2 md:gap-3 justify-end w-full md:w-auto">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        ...
                        className="bg-primary text-text-primary px-4 py-1.5 rounded-md hover:bg-primary/50 transition-colors cursor-pointer font-roboto text-base font-normal disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                      >
                        <DownloadIcon className="w-5 h-5" />
                        Download CSV
                      </button>
                  ...
                  <div className="relative bg-primary rounded-lg shadow-sm overflow-hidden h-full">
                    <div className="table-scrollbar overflow-x-auto overflow-y-auto h-[calc(100vh-332px)]">
                      <table className="w-full text-base text-left rtl:text-right text-text-primary">
                        <thead className="text-xs text-text-primary bg-primary border-b border-border-primary sticky top-0 z-10">
                          <tr>
                            <th className="px-4 py-3 text-text-primary uppercase text-start text-base font-roboto font-normal font-roboto">
                              Sr No
                            </th>
                            <th className="px-4 py-3 text-text-primary uppercase text-start text-base font-roboto font-normal font-roboto">
                              From Time
                            ...
                            <th className="px-4 py-3 text-text-primary text-start text-base font-roboto font-normal font-roboto">
                              <span className="uppercase">
                                Available Water(Start){" "}
                              </span>
                              <span className="italic">
